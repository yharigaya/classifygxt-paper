...
fig.dir <- "../figures/vis/sim/mcmc-bs/all"
fig.file1 <- "../figures/vis/sim/mcmc-bs/all/vln1.pdf"
fig.file2 <- "../figures/vis/sim/mcmc-bs/all/vln2.pdf"

args <- commandArgs(TRUE)
in.dir <- as.character(args[1])
fig.dir <- as.character(args[2])
fig.file1 <- as.character(args[3])
fig.file2 <- as.character(args[4])

type.vec <- c(
    "noranef-noranef", "noranef-ranef",
    "ranef-noranef", "ranef-ranef")
file.vec <- file.path(
    in.dir, type.vec, "vln.rds")
method.name <- c("log-NL", "log-LM", "RINT-LM")

dl.list <- file.vec %>% map(readRDS)

plot_vln <- function(dl) {
...
p.list <- dl.list %>% map(plot_vln)

(p.list[[1]] / p.list[[2]]) +
    plot_layout(guides="collect") &
    theme(legend.position="bottom") &
    plot_annotation(tag_levels="A")
ggsave(file=fig.file1, width=8, height=8.8)

(p.list[[3]] / p.list[[4]]) +
    plot_layout(guides="collect") &
    theme(legend.position="bottom") &
    plot_annotation(tag_levels="A")
ggsave(file=fig.file2, width=8, height=8.8)
